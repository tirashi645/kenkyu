...
def plot_generated_batch(X_proc, X_raw, generator_model, batch_size, suffix, epoch):
    ...
    plt.imshow(XX)
    plt.axis('off')
    plt.savefig(outputpath + "/current_batch_"+suffix+'_'+str(epoch)+".png")
    plt.clf()
    plt.close()
...
def train(epoch = 1000):
    ...
    for e in range(epoch):
        ...
        if (e+1)%100==0:
            ...
            generator_model.save(model_dir + '/generator_'+str((e+1))+'.h5')
            generator_model.save_weights(model_dir + '/generator_weights_'+str((e+1))+'.h5')
