def todo(path, classlist, original):
    ...
    random_color = np.random.randint(0, 255, (100, 3)).tolist()
    for label in classlist:
        ...
        for index, prev in enumerate(prev_points):
            ...
            flow_layer = cv2.circle(
                                            ...
                                            color = random_color[label[index]+1],
                                            thickness=3   # 線の太さ
                                        )
        ...
        cv2.imwrite('/media/koshiba/Data/opticalflow/point_data/result/' + str(videoName[:-4]) + '_GMM_class' + str(classNum) + '.jpg', frame2)
        classNum += 1
