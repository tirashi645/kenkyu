def todo(path, time):
    ...
    #保存するときのアルゴリズム名
    algorithm = 'GMM'
    # パラメータを指定する場合はこの変数をMeanshift()に代入する
    pram1 = dict(n_components=3, 
    ...
    pram3 = dict(n_components=3, 
                    ...
                    random_state=111, 
                    verbose=0)
    # クラスター数が多い場合はradom_colorを使用
    clist = ['gray', 'blue', 'orange', 'green', 'red', 'purple', 'brown', 'yellow', 'olive', 'lime', 'cyan', 'tan']
    random_color = np.concatenate([np.random.rand(100,3), np.array([[1.0]]*100)], 1)
    ...
    def class1_output(k_err, zahyou):
        ...
        clustering = GMM(**pram1).fit(zahyou_ave)

        # 特徴点ごと実行:フレーム数分k-meansで分類して一番多い分類を採用する
        labels = clustering.predict(vectors).tolist()
        print(max(labels))
        '''
        for pointNum, frame in enumerate(k_err):
        ...
        # プロットした画像を保存する
        plt.savefig('/media/koshiba/Data/opticalflow/point_data/plt/' + videoName[:-4] + '_GMM_class1_figure.png')
    ...
    def class2_output(hz_fft2, num_fft2):
        ...
        #分類対象のデータをクラスタリング
        clustering = GMM(**pram2)
        clustering.fit(vectors)
        label = clustering.predict(vectors)
        ...
        for i in range(len(vectors)):
            ...
            plt.scatter(vectors[:,0], vectors[:,1], color=random_color[label[i]+1], s=36)
        X, Y = np.meshgrid(np.linspace(-4, 4), np.linspace(-4,4))

        
        f = lambda x, y: np.exp(clustering.score(np.array([x, y])))
        ...
        fig.set_xlabel('vector in x', fontsize=36)
        fig.set_ylabel('vector in y', fontsize=36)
        #plt.tick_params(labelsize=36)
        #fig.show()
    ...
    def class3_output(hz_fft3, num_fft3):
        ...
        label = GMM(**pram3).fit_predict(vectors)
        ...
        for i in range(len(vectors)):
            #ax.scatter(vectors[i,0], vectors[i,1], color=clist[label[i]+1], s=36)
            ax.scatter(vectors[i,0], vectors[i,1], color=random_color[label[i]+1], s=36)
